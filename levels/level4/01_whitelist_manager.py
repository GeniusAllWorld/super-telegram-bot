import os
from aiogram import Router, F, html # Импортируем html для безопасного экранирования юзернеймов
...
async def show_whitelist(callback: CallbackQuery):
    # Жесткая проверка прав: пускаем только главного администратора
    ...
    if not whitelisted_users:
        ...
        for idx, u in enumerate(whitelisted_users, 1):
            # ПОФИКСИЛИ HTML-ИНЪЕКЦИЮ: Безопасно экранируем юзернеймы, чтобы не ломать парсер Telegram
            username_str = f"(@{html.quote(u.username)})" if u.username else ""
...
async def wl_add_trigger(callback: CallbackQuery, state: FSMContext):
    await callback.message.answer(
        "✏️ Введите <b>Telegram ID</b> пользователя, которого хотите ДОБАВИТЬ в белый список:\n"
...
async def process_wl_add(message: Message, state: FSMContext):
    ...
    if target_id_str.lower() in ["отмена", "/cancel"]:
        await state.clear()
        await message.answer("❌ Операция добавления в белый список отменена.")
        return
        
    if not target_id_str.isdigit():
        await message.answer("⚠️ ID должен состоять только из цифр! Попробуйте еще раз или напишите <b>отмена</b>.", parse_mode="HTML")
        return

    target_id = int(target_id_str)

    # ПОФИКСИЛИ КОНФЛИКТ КОНТЕКСТОВ: убран дублирующий session.begin(), используем явный commit()
    async with async_session_maker() as session:
        ...
        if not user:
            # Если юзера нет, создаем для него запись авансом
...
async def wl_remove_trigger(callback: CallbackQuery, state: FSMContext):
    await callback.message.answer(
        "✏️ Введите <b>Telegram ID</b> пользователя, которого хотите УДАЛИТЬ из белого списка:\n"
        "Для отмены операции напишите <b>отмена</b>.",
        parse_mode="HTML"
    )
...
async def process_wl_remove(message: Message, state: FSMContext):
    ...
    if target_id_str.lower() in ["отмена", "/cancel"]:
        await state.clear()
        await message.answer("❌ Операция удаления из белого списка отменена.")
        return
