...
TINYURL_API = "https://api.tinyurl.com/api-create.php?url="
...
async def start_shortener(callback: CallbackQuery, state: FSMContext):
    ...
    # Включаем состояние ожидания ссылки
...
async def process_shortener(message: Message, state: FSMContext):
    ...
    # Показываем статус "typing...", так как делаем сетевой запрос
    await message.bot.send_chat_action(chat_id=message.chat.id, action="typing")
    
    try:
        # Стучимся в API TinyURL
        async with aiohttp.ClientSession() as session:
            async with session.get(f"{TINYURL_API}{url}", timeout=5) as response:
                ...
                # API TinyURL в данном режиме возвращает просто строку с готовым URL
